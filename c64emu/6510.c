...
// Pr√∏ver oss p√• √• lage en mindre minnetabell (da det antas at SID filene ikke
// trenger det fulle 64kB i C64)
unsigned char *pages[256];	// pages, inneholder pekere til aktuelle minneomr√•der
...
void installSIDDriver(void) {
	...
	// for å hente rett IO Port konfigurasjon før vi kjører SID
	...
	storeMemRAMShort(address, 0x85, 0x1); address += 2;
	// JMP til oss selv, emulator koden skjønner hva vi skal frem til
...
//	resetMem();
	...
	// kj√∏r INIT adresse med sang i reg.a
