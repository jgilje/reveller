/*
    Emulering av MOS6510 (forh√•pentligvis)
	...
	- absolute (angir en adresse hvor innholdet skal hentes fra)
	  LDA $3E32
	- zero page (angir en adresse i page $00, sparer alts√• 1 byte i koden)
	  LDA $23
	...
	- indexed indirect (adderer x register til en en adresse (er denne kun zeropage?))
	  LDA ($B4,X)
	    ...
	    3. Accumulator = $EE12
	- indirect indexed (yarr!  alts√• motsatt av indexed indirect)
			   (henter ut indirect adresse, adderer s√• y til denne)
	  LDA ($B4),Y
	    1. henter ut $B4 (ex. $EE) og $B5 (ex. $12)
	    2. $12EE + Y (ex. $6) = $12EE + $6 = $12F4
	    3. Accumulator = $12F4
	- relativ (hopper frem eller tilbake, operand er en _signed_ byte)
		  (kan derfor hoppe i omr√•det -128 til +127)
    
    - Litt generelt dill
    
    - henter alltid inn to byte hver gang det leses fra minne (alts√• opcode og
      neste byte)
    - semi-pipelining: hvis forrige instruksjon ikke skrev til minne, s√• kan
      prosessoren hente inn neste opcode for neste instruksjon mens den utf√∏rer
      siste cpu-sykel.  Eks.: EOR #$FF:
        1. sykel: henter inn EOR ($49)
	2. sykel: henter inn parameter $FF
	3. sykel: utf√∏r og lagre instruksjon til Acc., men samtidig hentes
	          opcode for neste instruksjon
 
    - NMI og IRQ bruker 7 sykler.  IRQ og BRK setter I flagget, NMI gj√∏r ikke.
      Prosessoren gj√∏r seg ferdig med gjeldende instruksjon f√∏r den g√•r til
      Interrupt sekvensen.
*/
...
typedef struct Register {
    ...
    // index reg. x: hovedregister for Â addressere data med "indices".
    // kan addressere "indexed indirect" - lar en ha en vektor tabell pÂ
    // "Zero Page"
    unsigned char x;		// register x

    // index reg. y: kun dette har en indirect address mode som gir tilgang
    // til hele minnet uten Â bruke selv-modifiserende kode (skummelt...)
    unsigned char y;		// register y

    // Stack Pointer:
    //  256 bytes totalt stackminne (i omrÂdet 0x0100 - 0x01FF)
    //  S peker pÂ adresse i stackminnet, altsÂ (0x0100 + S)
    ...
    // for Â lese kun et flagg kan branch opcodes benyttes
    // for Â sette flagg kan P hentes fra Stack, eller en kan
    ...
    //	5	B: Break Flag (for Â skille mellom soft. og hard. interrupt)
    //		   Denne er alltid 1, bortsett fra nÂr P register blir PUSH -> S nÂr
    //		   en fÂr Jump til en hardware interrupt rutine. (Les ogsÂ om NMI og BRK)
    //	4	D: Decimal mode Flag (angir Decimal modus, denne er oftest 0)
    //		   Les spesielt pÂ ADC, SBC og ARR opcodes
    //	3	I: Interrupt disable Flag (hindrer CPU i Â gÂ til IRQ handle vector (0xFFFE)
...
// dette flagget bryter interpret funksjonen, typisk ved RTS i binærkoden
// (eks.: stack overflow)
unsigned char work;
...
// Definer reg.p sine flag, (giljen bare roter nÂr han skal trikse med dem)
#define FLAG_N 0x80
#define FLAG_V 0x40
